        ...
        <div class="title-outer">
          ...
          <template v-if="!loading">
            <SlotWheel
              ...
              ref="wheel"
              :items="santi"
              @stopped="runningWheel = false"
            />
            <div class="reveal-wrap">
              <button
                class="reveal-btn"
                @click="spinRandom"
                :disabled="runningWheel"
                aria-pressed="false"
              >
                {{ buttonLabel }}
              </button>
...
const props = defineProps({
  ...
  apiUrl: {
    type: String,
    default: 'https://santodelgiorno.mintdev.me/api/santo',
  },
...
const buttonLabel = computed(() => {
  if (!revealed.value) return 'Scopri il santo da jastemmare';
  if (runningWheel.value) return 'Jastemmando';
  return 'Jastemma un altro santo';
});
...
async function spinRandom() {
  ...
  runningWheel.value = true;
  const idx = Math.floor(Math.random() * santi.value.length);
  wheel.value.spinTo(idx);
}
...
async function fetchSanto() {
  ...
  try {
    ...
    santi.value = data.data.map((santo) => santo.attributes.nome);
    santo.value = {
...
.viva {
  ...
  color: #fff8ea;
  text-shadow: 0 6px 22px rgba(255, 238, 200, 0.06),
...
.reveal-btn {
  ...
  font-weight: 700;
  cursor: pointer;
  box-shadow: 0 8px 28px rgba(20, 30, 60, 0.45),
    0 0 28px rgba(255, 230, 140, 0.06) inset;
...
.reveal-btn:hover {
  ...
  box-shadow: 0 12px 40px rgba(20, 30, 60, 0.5);
}
.reveal-btn:active {
